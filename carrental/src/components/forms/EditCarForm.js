...
import {Form,Button, Message, Grid, Header, Segment,Checkbox,Dropdown} from 'semantic-ui-react';
...
class EditCarForm extends React.Component{
    state = {
      ...
      componentDidMount(){api.car.getById(this.props.id).then(res => this.setState({data: res}))}
    
      onSubmit = () => {
        const  errors = this.validate(this.state.data);
        this.setState({errors});
        let id=this.state.data.car_id;
        delete this.state.data.car_id;
        if(Object.keys(errors).length === 0){
          this.props
            .submit(id,this.state.data)
            .catch(err =>
              this.setState({ errors: {global: 'There was an error when editind this car'}, loading: false })
            );
        }
      };
      ...
      render(){
        ...
        return(
          ...
          <Grid.Column style={{ maxWidth: 850 }}>
            <Header as='h1' color='teal' textAlign='center'> Edit car</Header>
            <Form onSubmit={this.onSubmit}>
              ...
              <Segment style={{ display:'flex' }}>
                ...
                <Form.Field error={!!errors.power}>
                  ...
                  onChange={this.onChange}
                  />
                  {errors.power && <InlineError text={errors.location}/>}
                </Form.Field>
                ...
                <br/>
                <Grid.Column>
                <Button primary size='large' type='submit' onClick={this.onSubmit}>Edit this car</Button>
                </Grid.Column>
              </Segment>
            </Form>
          </Grid.Column>
